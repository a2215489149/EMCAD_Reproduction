...
本文主要分成兩個部分。前半段整理 EMCAD 想解決的問題，以及作者提出的方法為什麼有效；後半段則根據官方程式碼與這次實驗條件，說明 ClinicDB 與 Synapse 兩個資料集的復現結果。前者屬於二元分割，適合檢查模型是否能穩定回到論文數字；後者屬於多器官分割，任務難度更高，也更能看出模型在複雜情境下的表現。整體內容以論文方法理解、實驗設定與實際復現情況為主。
